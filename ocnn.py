...
class OneClassNeuralNetwork:
    ...
    def build_model(self):
        ...
        model = Sequential()
        input_hidden = Dense(h_size, input_dim=self.input_dim, kernel_initializer=RandomNormal(0.5), name="input_hidden")
        model.add(input_hidden)
        model.add(Activation(self.g))

        # Define Dense layer from hidden to output
        hidden_output = Dense(1, name="hidden_output", kernel_initializer=RandomNormal(0.5))
